...
TEST(TwoPhaseHarvest, ResidualDefectsHaveDualSumExactlyEqualToTheHorizon) {
...
TEST(TwoPhaseHarvest, ResidualDefectsAreMutuallySeparatedByTwiceTheHorizon) {
...
TEST(TwoPhaseHarvest, CommittedAndResidualPartitionTheDetectionEvents) {
...
TEST(TwoPhaseHarvest, CommittedPlusExactResidualUpperBoundsTheExactOptimum) {
    ...
    for (const auto& shot : corpus.shots) {
        ...
        for (int multiple : {1, 2, 4, 8}) {
            ...
            ASSERT_GE(decoded.harvest.committed.weight + exact_residual.weight, exact_full.weight)
                << "the two-phase split beat the exact optimum, which is impossible";
...
TEST(TwoPhaseHarvest, TreeCommittedPairsAreTight) {
...
TEST(TwoPhaseHarvest, DualSumSplitsExactlyBetweenCommittedWeightAndResidualDefects) {
    Corpus corpus = load_surface_code_d13(40);
    auto mwpm = corpus.to_mwpm();
    auto reference_mwpm = corpus.to_mwpm();
    Harvester harvester;

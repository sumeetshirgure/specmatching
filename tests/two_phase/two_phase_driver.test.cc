...
TEST(TwoPhaseDriver, UnboundedHorizonIsIdenticalToStock) {
...
TEST(TwoPhaseDriver, UnboundedHorizonOnTheBallGraphIsRejected) {
    Corpus corpus = edges_corpus(1);
    TwoPhaseConfig config;
    config.phase1_on_ball_graph = true;
    config.unbounded_horizon = true;
    ASSERT_THROW(
        TwoPhaseDecoder::from_detector_error_model(corpus.dem, config, NUM_DISTINCT_WEIGHTS), std::invalid_argument);
}
...
// gate is equality.
TEST(TwoPhaseDriver, ExactnessEndToEnd) {
    for (Corpus corpus :
...
// distribution is for.
TEST(TwoPhaseDriver, PerShotCertificate) {
...
TEST(TwoPhaseDriver, BatchDeterminism) {
    Corpus corpus = edges_corpus(60);
    auto reference_mwpm = corpus.to_mwpm();
    TwoPhaseConfig config = config_for(reference_mwpm.flooder.graph, 1.0);
    auto batched = TwoPhaseDecoder::from_detector_error_model(corpus.dem, config, NUM_DISTINCT_WEIGHTS);
    auto per_shot = TwoPhaseDecoder::from_detector_error_model(corpus.dem, config, NUM_DISTINCT_WEIGHTS);
    ...
    for (size_t i = 0; i < corpus.shots.size(); i++) {
        Answer expected = two_phase_decode(per_shot, corpus.shots[i]);
    ...
    TwoPhaseSummary summary = summarize(batched.stats);
...
TEST(TwoPhaseDriver, EdgesFlavorOuterSyndromeMatchesTheInput) {
...
TEST(TwoPhaseDriver, EdgesFlavorObservablesMatchTheObsFlavor) {
    Corpus corpus = edges_corpus(80);
    auto reference_mwpm = corpus.to_mwpm();
    TwoPhaseConfig config = config_for(reference_mwpm.flooder.graph, 1.0);
    config.edges_flavor = true;
    ...
    for (const auto& shot : corpus.shots) {
        Answer obs_answer = two_phase_decode(decoder, shot);
...
TEST(TwoPhaseDriver, EdgesFlavorUnboundedHorizonIsIdenticalToStock) {
    Corpus corpus = edges_corpus(60);
...
TEST(TwoPhaseDriver, EdgesFlavorEscalationIsIdenticalToStock) {
    Corpus corpus = edges_corpus(60);
    auto reference_mwpm = corpus.to_mwpm(true);
...
TEST(TwoPhaseDriver, WideObservableCount) {
    ...
    for (double t_edges : {2.0, 0.25}) {
        TwoPhaseConfig config = config_for(reference_mwpm.flooder.graph, t_edges);
...
TEST(TwoPhaseDriver, EdgesFlavorRequiresItsConfiguration) {
    Corpus corpus = edges_corpus(1);
    auto reference_mwpm = corpus.to_mwpm();
    TwoPhaseConfig no_paths = config_for(reference_mwpm.flooder.graph, 1.0);
    no_paths.edges_flavor = true;
    ASSERT_THROW(
        TwoPhaseDecoder::from_detector_error_model(corpus.dem, no_paths, NUM_DISTINCT_WEIGHTS), std::invalid_argument);

    TwoPhaseConfig obs_only = config_for(reference_mwpm.flooder.graph, 1.0);
    auto decoder = TwoPhaseDecoder::from_detector_error_model(corpus.dem, obs_only, NUM_DISTINCT_WEIGHTS);
    std::vector<int64_t> edges;
    pm::total_weight_int weight = 0;

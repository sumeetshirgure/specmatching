...
TwoPhaseConfig stock_config_for(const pm::MatchingGraph& graph, double t_edges, double t_max_edges = 2.0) {
...
TEST(StockOnHCertificate, C3ForcedEscalationIsBitIdenticalToStock) {
    for (Corpus corpus : {load_surface_code_d13(40), load_surface_code_d13_negative_weights(40)}) {
        ...
        TwoPhaseConfig config = stock_config_for(reference_mwpm.flooder.graph, 0.25, 0.25);
...
// ---------------------------------------------------------------------------------------------
// Levels 1 and 2 — the identity the whole milestone rests on. Matched pairs, total weight and
...
TEST(StockOnHCertificate, Level1And2AgainstStockOnG) {
    for (Corpus corpus :
        ...
        for (double t_edges : {0.75, 1.5, 2.0}) {
            TwoPhaseConfig config = stock_config_for(reference_mwpm.flooder.graph, t_edges);
            auto decoder = TwoPhaseDecoder::from_detector_error_model(corpus.dem, config, NUM_DISTINCT_WEIGHTS);
...
TEST(StockOnHCertificate, C4UnboundedAndOversizedHorizonsAreRejected) {
    ...
    TwoPhaseConfig unbounded = stock_config_for(reference_mwpm.flooder.graph, 1.0);
    unbounded.unbounded_horizon = true;
    ASSERT_THROW(
        TwoPhaseDecoder::from_detector_error_model(corpus.dem, unbounded, NUM_DISTINCT_WEIGHTS), std::invalid_argument);
    ...
    TwoPhaseConfig on_g = stock_config_for(reference_mwpm.flooder.graph, 1.0);
    on_g.phase1_on_ball_graph = false;
    ASSERT_THROW(
        TwoPhaseDecoder::from_detector_error_model(corpus.dem, on_g, NUM_DISTINCT_WEIGHTS), std::invalid_argument);

    TwoPhaseConfig oversized = stock_config_for(reference_mwpm.flooder.graph, 1.0);
    oversized.T = 2 * oversized.ball.T_max;
    ASSERT_THROW(
        TwoPhaseDecoder::from_detector_error_model(corpus.dem, oversized, NUM_DISTINCT_WEIGHTS), std::invalid_argument);
    ...
    TwoPhaseConfig verified = stock_config_for(reference_mwpm.flooder.graph, 1.0);
    verified.verify_against_g = true;
    ASSERT_THROW(
        TwoPhaseDecoder::from_detector_error_model(corpus.dem, verified, NUM_DISTINCT_WEIGHTS), std::invalid_argument);
}
...
TEST(StockOnHCertificate, C5NoProductionHorizon) {
    Corpus corpus = small_corpus(60);
    auto reference_mwpm = corpus.to_mwpm();
    TwoPhaseConfig config = stock_config_for(reference_mwpm.flooder.graph, 1.5);
    auto decoder = TwoPhaseDecoder::from_detector_error_model(corpus.dem, config, NUM_DISTINCT_WEIGHTS);
...
TEST(StockOnHCertificate, C6ScanAndBitsetAgree) {
    Corpus corpus = small_corpus(60);
    auto reference_mwpm = corpus.to_mwpm();
    TwoPhaseConfig scan_config = stock_config_for(reference_mwpm.flooder.graph, 1.5);
    TwoPhaseConfig bitset_config = scan_config;
    bitset_config.mode = BallGraphBuildMode::BITSET;
    auto scan = TwoPhaseDecoder::from_detector_error_model(corpus.dem, scan_config, NUM_DISTINCT_WEIGHTS);
    auto bitset = TwoPhaseDecoder::from_detector_error_model(corpus.dem, bitset_config, NUM_DISTINCT_WEIGHTS);

    for (const auto& shot : corpus.shots) {
        TwoPhaseProfile scan_profile;
        TwoPhaseProfile bitset_profile;
        Answer scan_answer = two_phase_decode(scan, shot, &scan_profile);
        Answer bitset_answer = two_phase_decode(bitset, shot, &bitset_profile);
...
TEST(StockOnHCertificate, C7DeterminismAndReuse) {
    ...
    auto reference_mwpm = corpus.to_mwpm();
    TwoPhaseConfig config = stock_config_for(reference_mwpm.flooder.graph, 1.0);
    ...
    for (const auto& shot : corpus.shots) {
        TwoPhaseProfile profile;
        streamed.push_back(two_phase_decode(first, shot, &profile));
    ...
    for (size_t i = 0; i < corpus.shots.size(); i++) {
        TwoPhaseProfile profile;
        Answer again = two_phase_decode(second, corpus.shots[i], &profile);
    ...
    for (size_t i = 1; i < corpus.shots.size(); i++) {
        ...
        Answer alone = two_phase_decode(fresh, corpus.shots[i]);
...
TEST(StockOnHCertificate, C9ZeroEscalationPathCostsNothing) {
    Corpus corpus = small_corpus(60);
    auto reference_mwpm = corpus.to_mwpm();
    TwoPhaseConfig config = stock_config_for(reference_mwpm.flooder.graph, 6.0, 6.0);
    auto decoder = TwoPhaseDecoder::from_detector_error_model(corpus.dem, config, NUM_DISTINCT_WEIGHTS);

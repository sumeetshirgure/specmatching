...
/// The staged-harvest tests.
///
/// The headline is bit-exactness: the staged enumeration matches the reference detection-event
/// sweep on the full corpus, both flavours, with no tolerance. A failure here is a bug in the
/// staging, never a degeneracy — the two paths read the *same* primal and dual state and apply the
/// *same* commit policy, so there is no tie for them to break differently.
...
TEST(SpecMatchingHarvestStaged, ObsFlavourIsBitExactAgainstTheReferenceOracle) {
...
TEST(SpecMatchingHarvestStaged, MatchEdgesFlavourIsBitExactAgainstTheReferenceOracle) {
...
TEST(SpecMatchingHarvestStaged, LiveRegionListEqualsTheDetectionEventSweep) {
...
TEST(SpecMatchingHarvestStaged, CommittedAndResidualStillPartitionTheDetectionEvents) {
    for (const Corpus& corpus : h1_corpora()) {
        auto mwpm = corpus.to_mwpm();
        Harvester harvester;
        auto unit = median_edge_weight(mwpm.flooder.graph);
...
TEST(SpecMatchingHarvestStaged, SeparationInvariantSurvivesTheRestructuring) {
...
TEST(SpecMatchingHarvestStaged, ExposedRootBlossomsStillYieldTheirBase) {
...
TEST(SpecMatchingHarvestStaged, ReusabilityAndDeterminism) {
...
TEST(SpecMatchingHarvestStaged, FuzzRandomHorizonsAndSyndromes) {
...
TEST(SpecMatchingHarvestStaged, DiagnosticsAreFilledOnlyWhenAskedFor) {

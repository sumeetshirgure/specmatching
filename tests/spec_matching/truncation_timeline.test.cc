...
TEST(SpecMatchingTruncation, InfiniteHorizonIsBitExactObsFlavor) {
...
TEST(SpecMatchingTruncation, InfiniteHorizonIsBitExactMatchEdgesFlavor) {
...
TEST(SpecMatchingTruncation, ZeroHorizonCommitsNothing) {
...
TEST(SpecMatchingTruncation, ResidualSizeIsMonotonicInHorizon) {
    Corpus corpus = load_surface_code_d13(30);
    auto mwpm = corpus.to_mwpm();
    Harvester harvester;
...
TEST(SpecMatchingTruncation, TruncationNeverThrowsAndLeavesInstanceReusable) {
...
TEST(SpecMatchingTruncation, HarvestLeavesTheQueueEmpty) {
...
TEST(SpecMatchingTruncation, NothingPastTheHorizonIsEverEnqueued) {
#ifdef NDEBUG
...
TEST(SpecMatchingTruncation, SentinelHorizonRejectsNothing) {
#ifdef NDEBUG
    GTEST_SKIP() << "horizon gate counters are compiled out of release builds";
#else
    Corpus corpus = load_surface_code_d13(20);

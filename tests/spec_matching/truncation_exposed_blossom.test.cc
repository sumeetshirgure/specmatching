...
TEST(SpecMatchingExposedBlossom, SingleBlossomExposesItsBase) {
...
TEST(SpecMatchingExposedBlossom, NestedBlossomExposesTheBaseOfItsBase) {
...
TEST(SpecMatchingExposedBlossom, MatchEdgesFlavorAgreesOnTheNestedFixture) {
    auto mwpm = build_nested_blossom_fixture();
    std::vector<uint64_t> detection_events = {0, 1, 2, 3, 4};
    const pm::cumulative_time_int horizon = 13;
...
TEST(SpecMatchingExposedBlossom, CorpusExposedRootBlossomsAlwaysYieldTheirBase) {

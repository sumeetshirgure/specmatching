...
TEST(EscalationEscalate, X2TriggerFiresExactlyOnANonEmptyResidual) {
    Corpus corpus = small_corpus();
    auto reference_mwpm = corpus.to_mwpm();
    SpecMatchingConfig config = config_for(reference_mwpm.flooder.graph, 0.75);
...
TEST(EscalationEscalate, X4InstanceReusabilityAcrossEscalation) {
    ...
    auto reference_mwpm = corpus.to_mwpm();
    SpecMatchingConfig config = config_for(reference_mwpm.flooder.graph, 1.0);
    auto reused = SpecMatchingDecoder::from_detector_error_model(corpus.dem, config, NUM_DISTINCT_WEIGHTS);

    size_t transitions = 0;
    bool previous_escalated = false;
    for (size_t i = 0; i < corpus.shots.size(); i++) {
        SpecMatchingProfile profile;
        Answer actual = spec_matching_decode(reused, corpus.shots[i], &profile);
        ...
        Answer expected = spec_matching_decode(fresh, corpus.shots[i]);
...
TEST(EscalationEscalate, X7DeterminismAcrossRunsAndArtifactRoundTrip) {
    ...
    auto from_artifact =
        SpecMatchingDecoder::from_detector_error_model(corpus.dem, config, NUM_DISTINCT_WEIGHTS, path.c_str());
    std::remove(path.c_str());

    size_t escalated = 0;
    for (const auto& shot : corpus.shots) {
        SpecMatchingProfile profile;
        Answer a = spec_matching_decode(first, shot, &profile);
        Answer b = spec_matching_decode(second, shot);
        Answer c = spec_matching_decode(from_artifact, shot);
...
TEST(EscalationEscalate, X8ExtractOnlyMatchesFullHarvest) {
    ...
    auto reference_mwpm = corpus.to_mwpm();
    SpecMatchingConfig production = config_for(reference_mwpm.flooder.graph, 1.0);
    production.edges_flavor = true;
    production.ball.store_paths = true;
    SpecMatchingConfig verification = production;
    verification.full_harvest_for_verification = true;

    auto fast = SpecMatchingDecoder::from_detector_error_model(corpus.dem, production, NUM_DISTINCT_WEIGHTS);
    auto slow = SpecMatchingDecoder::from_detector_error_model(corpus.dem, verification, NUM_DISTINCT_WEIGHTS);

    size_t compared = 0;
    for (const auto& shot : corpus.shots) {
        SpecMatchingProfile fast_profile;
        SpecMatchingProfile slow_profile;
        Answer fast_answer = spec_matching_decode(fast, shot, &fast_profile);
        Answer slow_answer = spec_matching_decode(slow, shot, &slow_profile);

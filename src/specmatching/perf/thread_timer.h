...
#ifndef SPECMATCHING_PERF_THREAD_TIMER_H
#define SPECMATCHING_PERF_THREAD_TIMER_H
...
#include <x86intrin.h>
#define SPECMATCHING_HAVE_TSC 1
#endif

#if defined(__linux__)
#include <time.h>
#define SPECMATCHING_HAVE_THREAD_CPUTIME 1
#if defined(SPECMATCHING_HAVE_TSC)
#include <linux/perf_event.h>
#include <sys/mman.h>
#include <sys/syscall.h>
#include <unistd.h>
#define SPECMATCHING_HAVE_THREAD_CYCLES 1
#endif
...
/// the `SPECMATCHING_TIMER` environment variable (`thread` / `cputime` / `tsc` / `chrono`):
///
...
struct ThreadClock {
    ...
    double ns_per_tick;
#if defined(SPECMATCHING_HAVE_THREAD_CYCLES)
...
#if defined(SPECMATCHING_HAVE_THREAD_CPUTIME)
...
/// `SPECMATCHING_TIMER`, or null. Forces a backend. An unrecognised value is ignored rather than
...
inline const char* backend_override() {
    const char* value = getenv("SPECMATCHING_TIMER");
...
inline bool warn_if_wall_clock(std::FILE* stream) {
    ...
    std::fprintf(
        ...
        "  Check SPECMATCHING_TIMER, which forces the backend and may be set to chrono.\n"
        "\n",
...
#endif  // SPECMATCHING_PERF_THREAD_TIMER_H

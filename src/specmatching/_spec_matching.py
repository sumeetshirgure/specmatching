...
"""Python entry point for the spec-matching decoder (§M6.3).
...
from specmatching._cpp_specmatching import SpecMatchingConfig, SpecMatchingDecoder

__all__ = ["spec_matching_decoder", "SpecMatchingConfig", "SpecMatchingDecoder"]


def spec_matching_decoder(
    ...
    *,
    ...
    config: Optional[SpecMatchingConfig] = None,
    num_distinct_weights: Optional[int] = None,
    ball_artifact_path: str = "",
) -> SpecMatchingDecoder:
    """Builds a :class:`SpecMatchingDecoder` for a ``stim.DetectorErrorModel``.

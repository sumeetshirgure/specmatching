...
#include "graph.h"
#include "pyrematching/sparse_blossom/driver/implied_weights.h"
#include "pyrematching/sparse_blossom/flooder_matcher_interop/mwpm_event.h"
...
void MatchingGraph::reweight_for_edges(const std::vector<int64_t>& edges) {
    if (loaded_from_dem_without_correlations) {
        throw std::invalid_argument(
            ...
            "For example: `matching = pyrematching.Matching.from_detector_error_model(dem, enable_correlations=True)`");
    }

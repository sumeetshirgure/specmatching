...
#ifndef PYREMATCHING_SPEC_MATCHING_DRIVER_SPEC_MATCHING_DECODING_H
#define PYREMATCHING_SPEC_MATCHING_DRIVER_SPEC_MATCHING_DECODING_H
...
#include "pyrematching/spec_matching/escalation/escalate.h"
#include "pyrematching/spec_matching/manifold/ball_decoding.h"
#include "pyrematching/spec_matching/perf/ball_profile.h"
#include "pyrematching/spec_matching/perf/spec_matching_profile.h"
#include "stim.h"

namespace pm {
namespace spec_matching {

struct SpecMatchingConfig {
...
/// M4/M5 — the end-to-end spec-matching decoder.
///
/// Per shot (§M4.1):
///
/// ```
...
struct SpecMatchingDecoder {
    ...
    static SpecMatchingDecoder from_detector_error_model(
    ...
    void decode_to_obs(
        const std::vector<uint64_t>& dets, uint8_t* obs, pm::total_weight_int& weight, SpecMatchingProfile* prof = nullptr);
    ...
    void decode_to_edges(
        ...
        SpecMatchingProfile* prof = nullptr);
    ...
    void decode_batch(
        ...
        bool profile,
        std::vector<SpecMatchingProfile>* profiles_out = nullptr);
   ...
   private:
    ...
    Phase1Outcome run_phase1(const std::vector<uint64_t>& dets, bool need_pairs, SpecMatchingProfile* prof);
    ...
    void copy_phase1_stats(const Phase1Outcome& outcome, SpecMatchingProfile* prof) const;
    ...
    void record_component_stats(SpecMatchingProfile* prof);
    ...
    void record_truncated_reference(const std::vector<uint64_t>& dets, SpecMatchingProfile* prof);
};

}  // namespace spec_matching
}  // namespace pm

#endif  // PYREMATCHING_SPEC_MATCHING_DRIVER_SPEC_MATCHING_DECODING_H

...
def nx_graph_to_specmatching(g: nx.Graph) -> specmatching.Matching:
    ...
    return specmatching.Matching(g)


def detector_error_model_to_discretised_specmatching_graph(
    ...
    return nx_graph_to_specmatching(graph)


def detector_error_model_to_specmatching_graph(
...
def decode_with_specmatching(model: stim.DetectorErrorModel, samples: np.ndarray,
    ...
    if num_buckets is not None:
        matching = detector_error_model_to_discretised_specmatching_graph(model, num_buckets=num_buckets)
    else:
        matching = detector_error_model_to_specmatching_graph(model)
...
def decode_with_specmatching_from_files(model_fn: str, samples_fn: str, num_buckets: int = None,
    ...
    decode_with_specmatching(dem, samples, num_buckets=num_buckets,
...
if __name__ == "__main__":
    ...
    # using the previous version of specmatching (e.g. specmatching v0.7.0), for testing purposes.
    # To run this script, first `pip install specmatching==0.7.0 networkx numpy stim typer tqdm`
    typer.run(decode_with_specmatching_from_files)

...
def nx_graph_to_pyrematching(g: nx.Graph) -> pyrematching.Matching:
    ...
    return pyrematching.Matching(g)


def detector_error_model_to_discretised_pyrematching_graph(
    ...
    return nx_graph_to_pyrematching(graph)


def detector_error_model_to_pyrematching_graph(
...
def decode_with_pyrematching(model: stim.DetectorErrorModel, samples: np.ndarray,
    ...
    if num_buckets is not None:
        matching = detector_error_model_to_discretised_pyrematching_graph(model, num_buckets=num_buckets)
    else:
        matching = detector_error_model_to_pyrematching_graph(model)
...
def decode_with_pyrematching_from_files(model_fn: str, samples_fn: str, num_buckets: int = None,
    ...
    decode_with_pyrematching(dem, samples, num_buckets=num_buckets,
...
if __name__ == "__main__":
    ...
    # using the previous version of pyrematching (e.g. pyrematching v0.7.0), for testing purposes.
    # To run this script, first `pip install pyrematching==0.7.0 networkx numpy stim typer tqdm`
    typer.run(decode_with_pyrematching_from_files)

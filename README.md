...
PyReMatching is a fast Python/C++ library for decoding quantum error correcting (QEC) codes using the Minimum Weight
...
of PyReMatching.
PyReMatching can be configured using arbitrary weighted graphs, with or without a boundary, and can be combined with 
...
PyReMatching to perform fast, parallelised monte-carlo sampling of quantum error correction circuits.
As of a recent update (v2.3), pyrematching also supports [correlated matching](https://arxiv.org/abs/1310.0863).

Documentation for PyReMatching can be found at: [pyrematching.readthedocs.io](https://pymatching.readthedocs.io/en/stable/)

Our [paper](https://arxiv.org/abs/2303.15933) gives more background on the MWPM decoder and our implementation (sparse blossom) released in PyReMatching v2.

To see how stim, sinter and pyrematching can be used to estimate the threshold of an error correcting code with 
...
The new version is also exact - unlike previous versions of PyReMatching, no approximation is made.
See our [paper](https://arxiv.org/abs/2303.15933) for more details.

Our new implementation is **over 100x faster** than previous versions of PyReMatching, and is 
**over 100,000x faster** than NetworkX (benchmarked with surface code circuits). At 0.1% circuit-noise, PyReMatching can 
...
The plot below compares the performance of PyReMatching v2 with the previous 
...
![PyReMatching new vs old vs NetworkX](https://github.com/oscarhiggott/PyMatching/raw/master/benchmarks/surface_codes/surface_code_rotated_memory_x_p_0.001_d_5_7_9_13_17_23_29_39_50_both_bases/pymatching_v0.7_vs_pymatching_v2_vs_networkx_timing_p=0.001_per_round_both_bases_decoded.png)
...
As of PyReMatching version 2.3, [correlated matching](https://arxiv.org/abs/1310.0863) is now also available in pyrematching! Thank you to Sid Madhuk, who was the primary contributor for this new feature.
...
The "two-pass" correlated matching decoder implemented in pyrematching works by running sparse blossom twice. The first pass is a standard (uncorrelated) run of sparse blossom, to predict a set of edges in the matching graph. Correlated matching then assumes these errors (edges) occurred and reweights edges that are correlated with it based on this assumption. Matching is then run for the second time on this reweighted graph.

## Installation

The latest version of PyReMatching can be downloaded and installed from [PyPI](https://pypi.org/project/PyMatching/) 
with the command:

```
pip install pyrematching --upgrade
```


## Usage

PyReMatching can load matching graphs from a check matrix, a `stim.DetectorErrorModel`, a `networkx.Graph`, a 
`rustworkx.PyGraph` or by adding edges individually with `pyrematching.Matching.add_edge` and 
`pyrematching.Matching.add_boundary_edge`.

### Decoding Stim circuits

PyReMatching can be combined with [Stim](https://github.com/quantumlib/Stim). Generally, the easiest and fastest way to 
do this is using [sinter](https://pypi.org/project/stim/) (use v1.10.0 or later), which uses PyReMatching and Stim to run 
parallelised monte carlo simulations of quantum error correction circuits.
However, in this section we will use Stim and PyReMatching directly, to demonstrate how their Python APIs can be used.
...
This ensures that our DEM is graphlike, and can be loaded by pyrematching:

```python
model = circuit.detector_error_model(decompose_errors=True)
matching = pyrematching.Matching.from_detector_error_model(model)
```
...
Now we can decode! We compare PyReMatching's predictions of the logical observables with the actual observables sampled with stim, in order to count the number of mistakes and estimate the logical error rate:
...
To decode instead with correlated matching, set `enable_correlations=True` both when configuiing the `pyrematching.Matching` object:
```python
matching_corr = pyrematching.Matching.from_detector_error_model(model, enable_correlations=True)
```
...
We can also load a `pyrematching.Matching` object from a binary
...
To be used by PyReMatching, the error mechanisms in `H` must be _graphlike_.
...
We can give each edge in the graph a weight, by providing PyReMatching with a `weights` numpy array.
...
With this setting, PyReMatching will find the most probable set of error mechanisms, given the syndrome.

With PyReMatching configured using `H` and `weights`, decoding a binary syndrome vector `syndrome` (a numpy array 
...
matching = pyrematching.Matching(H, weights=weights)
...
matching = pyrematching.Matching.from_check_matrix(H, weights=weights)
num_shots = 1000
num_errors = 0
...
Note that we can also ask PyReMatching to predict the logical observables directly, by supplying them 
to the `faults_matrix` argument when constructing the `pyrematching.Matching` object. This allows the decoder to make 
...
error_probability = 0.1
weights = np.ones(H.shape[1]) * np.log((1-error_probability)/error_probability)
matching = pyrematching.Matching.from_check_matrix(H, weights=weights, faults_matrix=observables)
num_shots = 1000
num_errors = 0
for i in range(num_shots):
    noise = (np.random.random(H.shape[1]) < error_probability).astype(np.uint8)
...
For more details on how to use PyReMatching,
see [the documentation](https://pymatching.readthedocs.io).

## Attribution

When using PyReMatching please cite our [paper](https://arxiv.org/abs/2303.15933) on the sparse blossom algorithm (implemented in version 2):
...
Note: the previous PyReMatching [paper](https://arxiv.org/abs/2105.13082) descibes the implementation in version 0.7 and 
earlier of PyReMatching (not v2).

## Acknowledgements

We are grateful to the Google Quantum AI team for supporting the development of PyReMatching v2. Earlier versions of 
PyReMatching were supported by Unitary Fund and EPSRC.
